...
def inject_css():
    ...
    /* Text inputs & text areas — white background, thin blue border (replaces
       the dark boxes Streamlit Cloud renders when it falls back to a dark
       theme for native widgets). */
    ...
    /* Selectboxes (Analysis Filters, Study selector, etc.) — white background,
       thin blue border on the closed control. */
    ...
    div[data-testid="stSelectbox"] svg {{ fill: {C_PRIMARY} !important; }}

    /* Selectbox / dropdown open menu — rendered in a portal at the document
       root, so it needs its own (non-scoped) rule. */
    ...
    /* Custom HTML data tables — replaces native st.dataframe rendering so
       colours are fully controlled: white background, thin blue border. */
    .rwe-table-wrap {{
        overflow-x: auto;
        margin-bottom: 1rem;
        border-radius: 6px;
    ...
    table.rwe-table tbody td {{
        background: {C_WHITE};
        color: #1A1A1A;
        padding: 0.5rem 0.8rem;
        border-bottom: 1px solid {C_BORDER};
    }}
    table.rwe-table tbody tr:hover td {{ background: #F7FBFE; }}

    /* Top-bar account menu (popover) — styled as plain text, not a button */
...
def render_table(df):
    """Render a DataFrame as a styled HTML table — white background, thin blue
    border — instead of the native st.dataframe widget. This sidesteps
    Streamlit Cloud's canvas-based data-grid, which can render with a dark
    theme regardless of the app's configured light theme."""
    html = df.to_html(index=False, border=0, classes="rwe-table", escape=True)
    st.markdown(f'<div class="rwe-table-wrap">{html}</div>', unsafe_allow_html=True)
...
def styled_fig(fig, title="", height=340):
    fig.update_layout(
        **CHART_LAYOUT,
        ...
        height=height,
        xaxis=dict(showgrid=True, gridcolor=C_GRID, linecolor="#1A1A1A", zeroline=False,
                   tickfont=dict(color="#1A1A1A"),
                   title=dict(font=dict(color="#1A1A1A"))),
        yaxis=dict(showgrid=True, gridcolor=C_GRID, linecolor="#1A1A1A", zeroline=False,
                   tickfont=dict(color="#1A1A1A"),
                   title=dict(font=dict(color="#1A1A1A"))),
    )
    return fig
...
def render_topbar(page_title=""):
    ...
    with col_home:
        if active != "landing":
            ...
            """, unsafe_allow_html=True)
    with col_right:
        st.markdown('<div style="padding-top:0.35rem; text-align:right;">', unsafe_allow_html=True)
        with st.popover("RWE123  |  Logged In  ▾", use_container_width=False):
            st.markdown(f"""
            <div style="font-size:0.8rem; color:{C_GRAY}; margin-bottom:0.5rem;">
              Signed in as <strong style="color:{C_DARK};">RWE123</strong>
            </div>
            """, unsafe_allow_html=True)
    ...
    if st.session_state.cohort_confirmed:
        ...
        if icd_data:
            ...
            def color_status(val):
                ...
                return ""
            render_table(icd_df.rename(columns={"code":"ICD-10 Code","description":"Description","status":"Inclusion Status"}))
        ...
        render_table(pd.DataFrame(prec_data))
...
def render_upload():
    ...
    with col_left:
        ...
        if uploaded:
            ...
            render_table(pd.DataFrame(profile_data))
        else:
...
def render_payer():
    ...
    with payer_tabs[0]:
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        render_table(payer_df.rename(columns={
...
def render_signal_lab():
    ...
    with sl_tabs[4]:
        ...
        render_table(pd.DataFrame(pred_data))
